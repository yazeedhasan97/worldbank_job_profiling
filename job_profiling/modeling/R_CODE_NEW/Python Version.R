...
statMoments = miniData %>% group_by(nodes) %>%drop_na(durations) %>% summarise(
  min     = min(durations ),
...
statMoments = statMoments %>% mutate(min     = scale(min), # , center = FALSE
                                     p50      = scale(p50),
...
controlBasic = rpart.control(minbucket = bucketSize, cp = 0.0002, minsplit = 2, maxsurrogate = 0, usesurrogate = 0, xval = 10) #  
controlBasic_ot = rpart.control(minbucket = bucketSize, cp = 0.0002, minsplit = 2, maxsurrogate = 0, usesurrogate = 0, xval = 10)
...
saveWorkbook(wb, 
             file = paste0('Results from Python.xlsx'), 
             overwrite = TRUE)
