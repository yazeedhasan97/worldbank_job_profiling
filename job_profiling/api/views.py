...
if os.path.exists(settings.MODELS_PATH):
    utils.API_ENCODERS = utils.load_encoders(utils.API_ENCODERS, path=None)
...
def get_cluster(request):
    ...
    data = data.dict()
    cluster, node = utils.predict(
        data,
        utils.API_CONTRIBUTORS,
        utils.API_ENCODERS,
        utils.API_ONEHOT_ENCODED,
        utils.API_LABEL_ENCODED,
        utils.API_ENCODERS_OUTPUT_ITEMS,
    )

    print('RES :', cluster)
    return JsonResponse({'cluster': str(cluster)}, safe=False, status=status.HTTP_200_OK)
